    ...
    <div class="max-w-4xl mx-auto px-6">
        <div class="text-center mb-12">
            ...
            <h1 class="text-4xl font-bold text-slate-900 mb-4 leading-tight">
                Edit <span class="text-gradient"> Post</span>
            </h1>
        ...
        <div class="bg-white rounded-2xl shadow-luxury overflow-hidden">
            ...
            <form class="p-8" action="/update" method="POST" enctype="multipart/form-data" novalidate>
                <?= csrf_field() ?>
                ...
                <div class="space-y-8">
                    ...
                    <div>
                        ...
                        <select
                            ...
                            <?php foreach ($categories as $category): ?>
                                <option value="<?= $category['id'] ?>" <?= old('categorie_id', $post['categorie_id'] ?? '') == $category['id'] ? 'selected' : '' ?>>
                                    <?= esc($category['name']) ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                    ...
                    <div>
                        ...
                        <textarea
                            ...
                            required
                            class="py-3 px-4 block w-full border border-slate-200 rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-purple-500"
                            placeholder="Write a compelling description..."><?= old('description', esc($post['description'] ?? '')) ?></textarea>
                    ...
                    <div>
                        ...
                        <input
                            ...
                            id="creation-date"
                            name="creation-date"
                            value="<?= old('creation-date', date('Y-m-d', strtotime($post['creation_date'] ?? ''))) ?>"
                            required
                            class="py-3 px-4 block w-full border border-slate-200 rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-purple-500">
                    ...
                    <div>
                        ...
                        <?php if (!empty($post['img_url'])): ?>
                            <div class="m-5">
                                <img src="<?= base_url('img_posts/' . esc(basename($post['img_url']))) ?>" alt="Post Image" class="w-32 h-32 object-cover rounded mt-2">
                            </div>
                        <?php endif; ?>
                        <input
                            type="file"
